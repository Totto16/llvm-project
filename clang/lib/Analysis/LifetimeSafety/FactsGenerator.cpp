...
llvm::SmallVector<Fact *> FactsGenerator::issuePlaceholderLoans() {
  ...
  if (auto ThisOrigins = FactMgr.getOriginMgr().getThisOrigins()) {
    ...
    const PlaceholderRoot *PR = FactMgr.getLoanMgr().getOrCreatePlaceholderRoot(
        cast<CXXMethodDecl>(FD));
    const Loan *L = FactMgr.getLoanMgr().createLoan(AccessPath(PR),
                                                    /*IssuingExpr=*/nullptr);
  ...
  for (const ParmVarDecl *PVD : FD->parameters()) {
    ...
    if (!List)
      continue;
    const PlaceholderRoot *PR =
        FactMgr.getLoanMgr().getOrCreatePlaceholderRoot(PVD);
    const Loan *L = FactMgr.getLoanMgr().createLoan(AccessPath(PR),
                                                    /*IssuingExpr=*/nullptr);
    PlaceholderLoanFacts.push_back(
        FactMgr.createFact<IssueFact>(L->getID(), List->getOuterOriginID()));
