...
int f11(size_t fine){
    ...
    //TODO
    return (int)1L + valid;
    // CHECK-MESSAGES: :[[@LINE-1]]:12: warning: cast to type 'int', should be rewritten into using a fixed type [totto-use-fixed-width-types-var]
}
...
float f17(double should_be_ok){
    ...
    //TODO: make this work
    char invalid_type[8] = {};
// CHECK-MESSAGES: :[[@LINE-1]]:10: warning: variable 'invalid_type' has type '(char)[]', which should be rewritten into using a fixed type [totto-use-fixed-width-types-var]

    int invalid_type2[2] = {};
// CHECK-MESSAGES: :[[@LINE-1]]:9: warning: variable 'invalid_type2' has type '(int)[]', which should be rewritten into using a fixed type [totto-use-fixed-width-types-var]
